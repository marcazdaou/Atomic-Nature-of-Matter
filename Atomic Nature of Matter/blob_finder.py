...
class BlobFinder:
    ...
    def getBeads(self, pixels):
        ...
        for i in range(b):
            if self._blobs[i].mass() >= pixels:  # mass of blob is greater than or equal to pixels
                a += [self._blobs[i]]  # add self._blobs to variable a
        return a
    ...
    def _findBlob(self, pic, tau, i, j, marked, blob):
        ...
        # Mark the pixel.
        if i < 0 or i >= pic.width():  # return if pixels i is out of bounds
            return
        if j < 0 or j >= pic.height():  # return if pixels j is out of bounds
            return
        if marked[i][j]:  # return marked pixel
            return
        if luminance.luminance(pic.get(i, j)) < tau:  # if luminance is less that tau
            return
        marked[i][j] = True  # mark pixel True
